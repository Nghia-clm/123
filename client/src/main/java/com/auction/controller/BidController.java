...
public class BidController implements Initializable {
    ...
    private void loadAutoBidStatusRequest() {
        ...
        Platform.runLater(() -> {
            // Kiểm tra điều kiện: chỉ cho phép thao tác auto-bid khi phiên đang RUNNING
            // và user là bidder, và phiên chưa kết thúc
            boolean canBid = Session.getInstance().isBidder()
                    && !auctionFinished
                    && "RUNNING".equals(statusLabel.getText());

            if (!canBid) {
                // Phiên chưa bắt đầu (OPEN), đã kết thúc, hoặc không phải bidder
                // → giữ nguyên trạng thái disabled, không enable bất kỳ nút nào
                return;
            }
