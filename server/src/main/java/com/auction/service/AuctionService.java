...
public class AuctionService {
    ...
    private Auction syncAuctionTimeState(Connection conn, Auction auction) throws SQLException {
        LocalDateTime now = LocalDateTime.now();
        AuctionStatus newStatus = computeTimeState(auction, now, false);
    ...
    /**
     * Tính trạng thái auction theo thời gian - read-only sync (có delay trước CANCELED).
     */
    private AuctionStatus computeTimeState(Auction auction, LocalDateTime now) {
        return computeTimeState(auction, now, true);
    }

    /**
     * @param applyDelay true  = chờ EMPTY_AUCTION_CANCEL_DELAY_SECONDS rồi mới CANCELED (read-only sync).
     *                   false = hết giờ + no winner -> CANCELED ngay (dùng trong placeBid).
     */
    private AuctionStatus computeTimeState(Auction auction, LocalDateTime now, boolean applyDelay) {
        AuctionStatus status = auction.getStatus();

        // Buoc 1: OPEN/RUNNING + het gio -> FINISHED
        ...
        // Buoc 2: FINISHED + no winner -> CANCELED (co the ap dung delay)
        if (status == AuctionStatus.FINISHED && auction.getCurrentWinnerId() == null) {
            boolean delayPassed = !applyDelay
